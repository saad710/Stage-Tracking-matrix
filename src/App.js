...
function App() {
  ...
  const stageTrack = (tempArray) => {
    ...
    tempArray.forEach((temp, index) => {
      ...
      const dateData = Math.abs(
        (new Date(firstStage.Last_Modified_Time).getTime() -
          new Date(temp.Last_Modified_Time).getTime()) /
          1000,
      )
  ...
  // console.log(result[0]["5188282000000006805"])

  let headKey = []
  result?.forEach((ob) => {
    headKey.push(...Object.keys(ob))
  })
  console.log(headKey)
  ...
  headKey?.forEach((val) => {
    result?.forEach((data) => {
      if (data[val] !== undefined) {
        // console.log(Object.assign({}, ...data[val]))
        tableValue.push(Object.assign({}, ...data[val]))
      }
    })
  })
  ...
  return (
    <div className="App">
      <StageTable result={result} headKey={headKey} tableValue={tableValue} />
    </div>
  )
}
